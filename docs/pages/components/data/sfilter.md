...
Фильтры. `SFilterGroup` управляет состоянием нескольких фильтров, а `SFilter` выступает оберткой для отдельных полей, автоматически обрабатывая привязку данных и дебаунс.
...
<SToggleGroup>
    <SToggle title="В чем отличие от аналогов?">
        ...
        <ol>
            ...
            <li>Убирать пустые значения из URL, чтобы сохранять консистентную ссылку.</li>
        </ol>
    </SToggle>
</SToggleGroup>
...
<br>Текущее значение: <code>{{ filter }}</code>

## Привязка к GET-параметрам

Часто бывает удобно привязать модель напрямую к GET-параметрам:
1. Взять стартовые значения модели из URL (без необходимости проброса через контроллер).
2. При изменениях фильтра автоматически обновлять URL (через Inertia `router.get`), при этом сбрасывая страницу (page) и сохраняя другие независимые параметры.

Для этого используется атрибут `bind-to-query`:
...
<br>Текущее значение: <code>{{ filter2 }}</code>.
Placeholder задает null-значение («не выбранный вариант»).
...
<br>Текущее значение: <code>{{ filter3 }}</code>
...
<br>Текущее значение: <code>{{ filter4 }}</code>
...
<br>Текущее значение: <code>{{ filter5 }}</code>

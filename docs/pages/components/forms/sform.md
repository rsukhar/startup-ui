...
::: tip
Декларативный сабмит через `method`/`action` использует Inertia-роутер. Зарегистрируйте его один раз при подключении плагина: `app.use(StartupUI, { router })` (см. [Миграцию](/pages/welcome/extras/migration#_6-inertiajs-стала-опциональной)). Без роутера используйте событие `@submit` (см. пример ниже).
:::

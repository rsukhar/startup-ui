...
Также не нужно вручную обрабатывать loading-состояние и выводить ошибки валидации — это уже встроено в логику компонентов.

## Собственный submit-метод

Если нужно задать собственную логику отправки, используйте событие `@submit`:
...
Используйте атрибут **hint** в `<SFormRow>` для вывода пояснительного текста:
...
Атрибут **titles-at-left** выносит заголовки в левую колонку. Ширину колонки можно задать через **titles-width**.
...
Если нужно задать модель вручную или разместить несколько компонентов в одной строке, используйте обычный `v-model`:
...
## Вывод кастомных ошибок

С помощью атрибута **error-key** можно указать, за какими ключами в массиве ошибок должен следить конкретный ряд формы.

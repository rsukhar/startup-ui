...
В атрибут `placeholder` подставляем текст, который выводится в лейбле, когда ни одна из ссылок не активна. Где `links` — это массив в формате `[{label, url, active}, ...]`.
...
По умолчанию в лейбле выводится текст активной ссылки, а если ни одна из ссылок не активна — плейсхолдер.

Когда нужно выводить фиксированный лейбл независимо от активной ссылки, задаем его одноименным атрибутом `label`. При этом, если на нем должна быть ссылка, используем атрибут `labelLink`:
...
Когда нужно задать кастомный лейбл, используем одноименный слот `#label`:
...
При желании можно задать ссылки не через `links`, а через слот:
...
Также можно использовать комбинированный вариант, когда часть ссылок задается атрибутом `links`, а часть — добавляется через слот:

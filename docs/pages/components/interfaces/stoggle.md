...
Раскрывающийся блок (аккордеон). Позволяет скрывать и показывать контент. `SToggle` может использоваться как самостоятельно, так и внутри `SToggleGroup`.
...
Чтобы блок был открыт при загрузке страницы, добавьте атрибут `opened`:
...
Для сложной разметки в заголовке используйте слот `#title`:
...
## Группа блоков (Аккордеон)

Когда несколько `SToggle` обернуты в `SToggleGroup`, по умолчанию одновременно может быть открыт только один блок (при открытии одного другие закрываются).
...
Если нужно разрешить открытие нескольких блоков одновременно, добавьте атрибут `multiple`:

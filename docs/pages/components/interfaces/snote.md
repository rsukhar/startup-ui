...
Заметки могут иметь различные цветовые темы для обозначения разного семантического смысла:
...
Можно указать `icon` и `title`, чтобы сделать заметку более информативной. В основном слоте может быть любой HTML-контент:

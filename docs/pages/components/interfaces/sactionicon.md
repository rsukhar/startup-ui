...
const code3 = `<SActionIcon icon="pen-to-square" title="Редактировать" :is="Link" href="https://suhar.ru" />
`;
...
const code4 = `<SActionIcon title="Удалить" @click="deleteUser(user.username)" icon="trash" danger confirm="\`Вы действительно хотите удалить пользователя?\`" />
`;

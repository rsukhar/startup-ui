...
Передайте массив элементов в проп `items` и определите содержимое для каждого элемента с помощью слота `#item`. Слот дает доступ к текущему `item` и его индексу `index`.

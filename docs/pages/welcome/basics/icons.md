...
FontAwesome **опциональна**: собственная «обвязка» компонентов (шевроны, крестики, карет, бургер и т.п.) рисуется встроенными inline-SVG. FA нужна только для иконок, которые вы передаёте сами через проп `icon` — это `SActionIcon`, `SNote`, `SStatus`, а также опциональные иконки `STooltip` и `SDatePicker`.

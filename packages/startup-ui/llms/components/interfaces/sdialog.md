...
```vue
<template>
    <SDialog v-model="isShown" title="Уведомления" not-modal :position="{ top: '5rem', right: '2rem' }">
        <p>Окно закреплено в правом верхнем углу.</p>
        <p>Его по-прежнему можно перетащить за шапку.</p>
    </SDialog>
